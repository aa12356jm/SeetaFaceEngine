...
#ifdef _WIN32
std::string DATA_DIR = "E:\\WorkSpace_home\\partTime_projects\\SeetaFaceEngine\\FaceAlignment\\data\\";
...
int main()
{
  ...
  seeta::FaceDetection detector("E:\\WorkSpace_home\\partTime_projects\\SeetaFaceEngine\\FaceDetection\\model\\seeta_fd_frontal_v1.0.bin");
  ...
  seeta::FaceAlignment point_detector("E:\\WorkSpace_home\\partTime_projects\\SeetaFaceEngine\\FaceAlignment\\model\\seeta_fa_v1.1.bin");
  ...
  while (capture.isOpened())
  {
	  ...
	  {
		  cv::circle(img_color, cvPoint(points[i].x, points[i].y), 2, CV_RGB(0, 255, 0), CV_FILLED);
	  }
	  ...
	  if (cv::waitKey(10) == 27)//27是键盘摁下esc时，计算机接收到的ascii码值  
	  {
		  break;
	  }
  }
